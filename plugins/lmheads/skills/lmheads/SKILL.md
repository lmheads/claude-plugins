...
If your client doesn't expose a live channel surface (Claude Desktop /
Cowork build), follow the "Scheduled polling" section below instead.

---

## Scheduled polling (Claude Desktop / Cowork)

The Cowork build of this plugin does not declare a `channels[]`
capability — Cowork has no in-session push, so async updates can't
arrive on their own. The strategy: **register a Cowork scheduled task
that polls every 30 seconds whenever there's an open task to track.**

### When to register the schedule

Immediately after **any** of these tool calls, while still in the user's
turn:
- `start_task` — the user is now waiting on a callee's reply.
- `delegate_task` — same, but the caller is the agent itself.
- `respond_to_task` with `state="input-required"` — the agent is now
  waiting on the original caller's clarification.

If you skip this step, the user will not see updates until they next
talk to you.

### How to register

Use Claude Desktop's scheduled-task feature. The schedule should fire
every 30 seconds and invoke the `lmheads-cowork-watcher` subagent.

Use a stable, identifiable name like `lmheads-poll` so you can detect
duplicates. Idempotency rule: **before creating, check whether a task
with that name already exists; if it does, don't create another.**

### What the schedule should do

The `lmheads-cowork-watcher` subagent (shipped with the Cowork build)
encapsulates one tick of the poll: list inbound + outbound tasks for
each owned agent, surface deltas, and report whether any task is still
non-terminal. You don't need to write that logic inline; just point the
schedule at the subagent.

### Cleanup

After every tick, the watcher reports whether any task is still in a
non-terminal state (`submitted`, `working`, `input-required`,
`auth-required`). When the report says **all tasks are terminal**,
remove the scheduled task — keeping it running burns cycles for nothing.

### What you'll see between ticks

Native macOS notifications won't fire in the Cowork build (no streamer
runs locally). You'll learn about updates only when the watcher's tick
surfaces them. That's why a 30-second interval is the right default —
faster wastes budget, slower feels broken.
